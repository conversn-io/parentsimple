...
import { formatPhoneForInput, formatE164 } from '@/utils/phone-utils'
...
export function LifeInsuranceCAQuiz() {
  ...
  return (
    <div className="min-h-screen bg-[#F9F6EF]">
      ...
      <main className="max-w-lg mx-auto px-4 py-8 pb-16">
        ...
        {isContactStep && (
          <>
            ...
            <p className="text-sm text-red-600 font-medium mb-2 flex items-center justify-center gap-1">
              <span>✓</span> Great, you&apos;re matched with 13+ insurers
            </p>
            ...
            <form onSubmit={handleContactSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <span className="inline-flex items-center px-3 py-3 border border-gray-300 rounded-xl bg-gray-50 text-gray-600 text-sm">
                    +1
                  </span>
                  <input
                    type="tel"
                    ...
                    className="flex-1 px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500/20 focus:border-red-500"
              ...
              <p className="text-xs text-gray-500">
                ...
                <Link href="/terms-of-service" className="text-red-600 underline">Terms of Service</Link>
                {' '}and{' '}
                <Link href="/privacy-policy" className="text-red-600 underline">Privacy Policy</Link>.
              </p>
              <div className="bg-red-50 rounded-xl p-4 flex items-center gap-3">
                <span className="text-red-600">🕐</span>
                <span className="font-medium text-gray-900">Submit now, get covered in 24 hours</span>
                <span className="text-green-600 ml-auto">✓</span>
              </div>
              <p className="text-sm text-gray-500 text-center">Your personalized quotes are ready</p>
              <button
                type="submit"
                disabled={isSubmitting}
                className={`w-full py-4 rounded-xl font-bold text-white ${ACCENT_BG} hover:opacity-95 disabled:opacity-50`}
              >
      ...
      {/* Trust badges - show on all steps except contact (or show on all) */}
      <footer className="border-t border-gray-100 bg-gray-50/50 py-6 px-4">
        <div className="max-w-lg mx-auto flex flex-wrap justify-center gap-6 text-sm text-gray-600">
          <span className="flex items-center gap-2">
            <span className="text-red-600">🛡</span> No medical exam
          </span>
          <span className="flex items-center gap-2">
            <span className="text-red-600">🕐</span> No waiting period
          </span>
          <span className="flex items-center gap-2">
            <span className="text-green-600">✓</span> No spam, No pressure
          </span>
        </div>
      </footer>
    </div>
